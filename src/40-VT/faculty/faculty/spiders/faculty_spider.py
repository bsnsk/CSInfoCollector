...
class FacultySpider(scrapy.Spider):
    ...
    def person_info_parse(self, response):
        ...
        if filename == "":
            return
        target_filename = "../../../data/40-VT/" + name.encode('utf-8') + "/" + filename.lstrip('/').encode('utf-8')
    ...
    def person_index_parse(self, response):
        ...
        if name == "":
            return
        filename = "../../../data/40-VT/" + name.encode('utf-8') + "/index.html"
        print response.url,
        if os.path.isfile(filename):
            ...
            print " ..."
        system('mkdir "../../../data/40-VT/' + name.encode('utf-8') + '"')
        ...
        for sel in response.xpath('//a/@href'):
            ...
            if url.find(":") == -1 and not (url[-4:] in [".pdf", ".avi", ".mp4", ".mov", ".jpg", ".JPG", ".png", ".ppt"])\
                    and not (url[-3:] in [".ps"]):
    ...
    def parse(self, response):
        for sel in response.xpath('//*[@id="main-content"]/div/table/tr/td[2]'):
            name = sel.xpath("h3/a/text()").extract()[0]
            link = sel.xpath("div[1]/table[1]/tr[4]/td/a/@href").extract()[0]
            if name.strip() == "":
                continue
            if link.find(":") == -1:

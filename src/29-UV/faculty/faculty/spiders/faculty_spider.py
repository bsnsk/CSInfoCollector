...
class FacultySpider(scrapy.Spider):
    ...
    def person_info_parse(self, response):
        ...
        if filename == "":
            return
        target_filename = "../../../data/29-UV/" + name.encode('utf-8') + "/" + filename.lstrip('/').encode('utf-8')
    ...
    def person_index_parse(self, response):
        ...
        if name == "":
            return
        filename = "../../../data/29-UV/" + name.encode('utf-8') + "/index.html"
        print response.url,
        if os.path.isfile(filename):
            ...
            print " ..."
        system('mkdir "../../../data/29-UV/' + name.encode('utf-8') + '"')
    ...
    def parse(self, response):
        for sel in response.xpath('//table/tr/td'):
            if len(sel.xpath("h3").extract()) == 0:
                continue
            name = sel.xpath("h3/text()").extract()[0]
            if len(sel.xpath("p/a[contains(@href, 'http')]").extract()) == 0:
                continue
            link = sel.xpath("p/a[contains(@href, 'http')]/@href").extract()[0] 
            if name.strip() == "":
                continue
            if link.find(":") == -1:

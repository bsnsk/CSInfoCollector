...
class FacultySpider(scrapy.Spider):
    ...
    def person_info_parse(self, response):
        ...
        if filename == "":
            return
        target_filename = "../../../data/1-CMU/" + name.encode('utf-8') + "/" + filename.lstrip('/').encode('utf-8')
    ...
    def person_index_parse(self, response):
        ...
        if name == "":
            return
        filename = "../../../data/1-CMU/" + name.encode('utf-8') + "/index.html"
        print response.url,
        if os.path.isfile(filename):
            ...
            print " ..."
        system('mkdir "../../../data/1-CMU/' + name.encode('utf-8') + '"')
...
#                yield scrapy.Request(url, meta={'item':item}, callback=self.person_info_parse)
    ...
    def parse(self, response):
        # even
        for sel in response.xpath('//table/tbody/*[contains(concat(" ", normalize-space(@class), " "), " even ")]'):
            name = " ".join(sel.xpath('td/a/text()').extract()[:2])
        ...
        # odd
        for sel in response.xpath('//table/tbody/*[contains(concat(" ", normalize-space(@class), " "), " odd ")]'):
            name = " ".join(sel.xpath('td/a/text()').extract()[:2])
            link = sel.xpath('td/a/@href').extract()[0]
            if name.strip() == "":
                continue

...
class FacultySpider(scrapy.Spider):
    ...
    def person_info_parse(self, response):
        ...
        if filename == "":
            return
        target_filename = "../../../data/48-ASU/" + name.encode('utf-8') + "/" + filename.lstrip('/').encode('utf-8')
    ...
    def person_index_parse(self, response):
        ...
        if name == "":
            return
        filename = "../../../data/48-ASU/" + name.encode('utf-8') + "/index.html"
        print response.url,
        if os.path.isfile(filename):
            ...
            print " ..."
        system('mkdir "../../../data/48-ASU/' + name.encode('utf-8') + '"')
    ...
    def parse(self, response):
        for sel in response.xpath('//*[contains(concat(" ", normalize-space(@class), " "), " portfolio_title ")]'):

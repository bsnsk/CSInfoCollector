...
class FacultySpider(scrapy.Spider):
    ...
    start_urls = [
            "https://engineering.tamu.edu/cse/people/faculty"
    ]
    ...
    def person_info_parse(self, response):
        ...
        if filename == "":
            return
        target_filename = "../../../data/40-TAMU/" + name.encode('utf-8') + "/" + filename.lstrip('/').encode('utf-8')
    ...
    def person_index_parse(self, response):
        ...
        if name == "":
            return
        filename = "../../../data/40-TAMU/" + name.encode('utf-8') + "/index.html"
        print response.url,
        if os.path.isfile(filename):
            ...
            print " ..."
        system('mkdir "../../../data/40-TAMU/' + name.encode('utf-8') + '"')
    ...
    def parse(self, response):
        for sel in response.xpath('//div[contains(concat(" ", normalize-space(@class), " "), " record medium-6 columns ")]/h4'):

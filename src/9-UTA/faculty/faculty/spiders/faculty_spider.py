...
class FacultySpider(scrapy.Spider):
    ...
    def person_info_parse(self, response):
        ...
        if filename == "":
            return
        target_filename = "../../../data/9-UTA/" + name.encode('utf-8') + "/" + filename.lstrip('/').encode('utf-8')
    ...
    def person_index_parse(self, response):
        ...
        if name == "":
            return
        filename = "../../../data/9-UTA/" + name.encode('utf-8') + "/index.html"
        print response.url,
        if os.path.isfile(filename):
            ...
            print " ..."
        system('mkdir "../../../data/9-UTA/' + name.encode('utf-8') + '"')
...
#                yield scrapy.Request(url, meta={'item':item}, callback=self.person_info_parse)
    ...
    def parse(self, response):
        for sel in response.xpath('//*[contains(concat(" ", normalize-space(@class), " "), " field-content ")]'):
            if len(sel.xpath("a").extract()) == 0:
                continue
            name = sel.xpath('*[contains(concat(" ", normalize-space(@class), " "), " prof-name ")]/text()').extract()[0]

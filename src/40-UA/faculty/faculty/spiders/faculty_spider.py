...
class FacultySpider(scrapy.Spider):
    ...
    start_urls = [
            "https://www.cs.arizona.edu/personnel/faculty.html"
    ]
    ...
    def person_info_parse(self, response):
        ...
        if filename == "":
            return
        target_filename = "../../../data/40-UA/" + name.encode('utf-8') + "/" + filename.lstrip('/').encode('utf-8')
    ...
    def person_index_parse(self, response):
        ...
        if name == "":
            return
        filename = "../../../data/40-UA/" + name.encode('utf-8') + "/index.html"
        print response.url,
        if os.path.isfile(filename):
            ...
            print " ..."
        system('mkdir "../../../data/40-UA/' + name.encode('utf-8') + '"')
    ...
    def parse(self, response):
        for sel in response.xpath('//div[contains(concat(" ", normalize-space(@class), " "), " notes ")]/following-sibling::a[1]'):

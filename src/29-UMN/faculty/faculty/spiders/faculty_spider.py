...
class FacultySpider(scrapy.Spider):
    ...
    def person_info_parse(self, response):
        ...
        if filename == "":
            return
        target_filename = "../../../data/29-UMN/" + name.encode('utf-8') + "/" + filename.lstrip('/').encode('utf-8')
    ...
    def person_index_parse(self, response):
        ...
        if name == "":
            return
        filename = "../../../data/29-UMN/" + name.encode('utf-8') + "/index.html"
        print response.url,
        if os.path.isfile(filename):
            ...
            print " ..."
        system('mkdir "../../../data/29-UMN/' + name.encode('utf-8') + '"')
    ...
    def parse(self, response):
        for sel in response.xpath('//table/tbody/tr/td'):
            if len(sel.xpath("*[contains(@class,'views-field') and contains(@class ,'views-field-field-quick-links-links')]/div/a/text()").extract()) == 0:
                continue
            textname = sel.xpath("*[contains(@class,'views-field') and contains(@class ,'views-field-field-quick-links-links')]/div/a/text()").extract()[0]
            if textname != "Homepage":
                continue
            name = sel.xpath("*[contains(@class,'views-field') and contains(@class ,'views-field-title')]/strong/text()").extract()[0]
            link = sel.xpath("*[contains(@class,'views-field') and contains(@class ,'views-field-field-quick-links-links')]/div/a/@href").extract()[0]
            print name
            print link
            if name.strip() == "":
                continue

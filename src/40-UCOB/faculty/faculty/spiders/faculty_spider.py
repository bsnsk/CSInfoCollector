...
class FacultySpider(scrapy.Spider):
    name = "faculty"
    allowed_domains = ["colorado.edu"]
    start_urls = [
            "http://www.colorado.edu/cs/our-people?field_person_type_tid=1"
    ]
    ...
    def person_info_parse(self, response):
        ...
        if filename == "":
            return
        target_filename = "../../../data/40-UCOB/" + name.encode('utf-8') + "/" + filename.lstrip('/').encode('utf-8')
    ...
    def person_index_parse(self, response):
        ...
        if name == "":
            return
        filename = "../../../data/40-UCOB/" + name.encode('utf-8') + "/index.html"
        print response.url,
        if os.path.isfile(filename):
            ...
            print " ..."
        system('mkdir "../../../data/40-UCOB/' + name.encode('utf-8') + '"')
    ...
    def parse(self, response):
        for sel in response.xpath('//table/tbody/tr/td/a'):
            if len(sel.xpath("./@href").extract()) == 0:
                continue
            if len(sel.xpath("./div[contains(@class,'field-name-field-first-name')]/div/div").extract())==0:
                continue
            link = sel.xpath("./@href").extract()[0]
            first_name = sel.xpath("./div[contains(@class,'field-name-field-first-name')]/div/div/text()").extract()[0]
            last_name = sel.xpath("./div[contains(@class, 'field-name-field-last-name')]/div/div/text()").extract()[0]

...
class FacultySpider(scrapy.Spider):
    ...
    def person_info_parse(self, response):
        ...
        if filename == "":
            return
        target_filename = "../../../data/34-UCSB/" + name.encode('utf-8') + "/" + filename.lstrip('/').encode('utf-8')
    ...
    def person_index_parse(self, response):
        ...
        if name == "":
            return
        filename = "../../../data/34-UCSB/" + name.encode('utf-8') + "/index.html"
        print response.url,
        if os.path.isfile(filename):
            ...
            print " ..."
        system('mkdir "../../../data/34-UCSB/' + name.encode('utf-8') + '"')
    ...
    def parse(self, response):
        for sel in response.xpath('//table/tbody/tr'):
            name = sel.xpath("td[contains(@class, 'views-field-title')]/a/text()").extract()[0]
            if len(sel.xpath("td[contains(@class, 'views-field-field-website-link')]/a/@href").extract()) == 0:
                continue
            link = sel.xpath("td[contains(@class, 'views-field-field-website-link')]/a/@href").extract()[0]
            if name.strip() == "":
                continue
            if link.find(":") == -1:

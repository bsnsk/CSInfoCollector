...
class FacultySpider(scrapy.Spider):
    name = "faculty"
    allowed_domains = ["colostate.edu"]
    start_urls = [
            "http://www.cs.colostate.edu/cstop/cspeople/csfacultyandstaff.php"
    ]
    ...
    def person_info_parse(self, response):
        ...
        if filename == "":
            return
        target_filename = "../../../data/70-CSU/" + name.encode('utf-8') + "/" + filename.lstrip('/').encode('utf-8')
    ...
    def person_index_parse(self, response):
        ...
        if name == "":
            return
        filename = "../../../data/70-CSU/" + name.encode('utf-8') + "/index.html"
        print response.url,
        if os.path.isfile(filename):
            ...
            print " ..."
        system('mkdir "../../../data/70-CSU/' + name.encode('utf-8') + '"')
    ...
    def parse(self, response):
        for sel in response.xpath('//tr/td/p'):
            if len(sel.xpath("a").extract()) == 0:
                continue
            name = sel.xpath("a/strong/text()").extract()[0]

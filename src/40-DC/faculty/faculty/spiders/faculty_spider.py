...
class FacultySpider(scrapy.Spider):
    name = "faculty"
    allowed_domains = ["dartmouth.edu"]
    start_urls = [
            "http://web.cs.dartmouth.edu/people"
    ]
    ...
    def person_info_parse(self, response):
        ...
        if filename == "":
            return
        target_filename = "../../../data/40-DC/" + name.encode('utf-8') + "/" + filename.lstrip('/').encode('utf-8')
    ...
    def person_index_parse(self, response):
        ...
        if name == "":
            return
        filename = "../../../data/40-DC/" + name.encode('utf-8') + "/index.html"
        print response.url,
        if os.path.isfile(filename):
            ...
            print " ..."
        system('mkdir "../../../data/40-DC/' + name.encode('utf-8') + '"')
    ...
    def parse(self, response):
        for sel in response.xpath('//*[contains(@id, "person-name")]'):

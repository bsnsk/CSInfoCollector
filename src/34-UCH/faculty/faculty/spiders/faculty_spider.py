...
class FacultySpider(scrapy.Spider):
    ...
    start_urls = [
            "https://www.cs.uchicago.edu/directories/table/faculty"
    ]
    ...
    def person_info_parse(self, response):
        ...
        if filename == "":
            return
        target_filename = "../../../data/34-UCH/" + name.encode('utf-8') + "/" + filename.lstrip('/').encode('utf-8')
    ...
    def person_index_parse(self, response):
        ...
        if name == "":
            return
        filename = "../../../data/34-UCH/" + name.encode('utf-8') + "/index.html"
        print response.url,
        if os.path.isfile(filename):
            ...
            print " ..."
        system('mkdir "../../../data/34-UCH/' + name.encode('utf-8') + '"')
    ...
    def parse(self, response):
        for sel in response.xpath('//table/tbody/tr'):
            name = sel.xpath("td/text()").extract()[0].strip()
            if len(sel.xpath("td/p/a[text() = 'Personal website']").extract()) == 0:
                continue
            link = sel.xpath("td/p/a[text() = 'Personal website']/@href").extract()[0]
            print name
            print link
            if name.strip() == "":
                continue

...
class FacultySpider(scrapy.Spider):
    ...
    start_urls = [
            "http://cse.wustl.edu/PEOPLE/Pages/tenured-tenure-track-faculty.aspx"
    ]
    ...
    def person_info_parse(self, response):
        ...
        if filename == "":
            return
        target_filename = "../../../data/40-WUSTL/" + name.encode('utf-8') + "/" + filename.lstrip('/').encode('utf-8')
    ...
    def person_index_parse(self, response):
        ...
        if name == "":
            return
        filename = "../../../data/40-WUSTL/" + name.encode('utf-8') + "/index.html"
        print response.url,
        if os.path.isfile(filename):
            ...
            print " ..."
        system('mkdir "../../../data/40-WUSTL/' + name.encode('utf-8') + '"')
    ...
    def parse(self, response):
        for sel in response.xpath('//*[@id="MainContentZone"]/div/table/tbody/tr/td/a[2]'):
            if len(sel.xpath(".").extract()) == 0:
                continue

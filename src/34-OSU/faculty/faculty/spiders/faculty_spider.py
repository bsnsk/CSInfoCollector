...
class FacultySpider(scrapy.Spider):
    name = "faculty"
    allowed_domains = ["cse.osu.edu","cse.ohio-state.edu"]
    start_urls = [
    ...
    def person_info_parse(self, response):
        ...
        if filename == "":
            return
        target_filename = "../../../data/34-OSU/" + name.encode('utf-8') + "/" + filename.lstrip('/').encode('utf-8')
    ...
    def person_index_parse(self, response):
        ...
        if name == "":
            return
        filename = "../../../data/34-OSU/" + name.encode('utf-8') + "/index.html"
        print response.url,
        if os.path.isfile(filename):
            ...
            print " ..."
        system('mkdir "../../../data/34-OSU/' + name.encode('utf-8') + '"')
    ...
    def parse(self, response):
        for sel in response.xpath('//*[contains(concat(" ", normalize-space(@class), " "), " profile-details ")]'):
            if len(sel.xpath("h2/a").extract()) == 0:
                continue

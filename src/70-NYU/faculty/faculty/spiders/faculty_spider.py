...
class FacultySpider(scrapy.Spider):
    ...
    start_urls = [
            "http://cs.nyu.edu/dynamic/people/faculty/"
    ]
    ...
    def person_info_parse(self, response):
        ...
        if filename == "":
            return
        target_filename = "../../../data/70-NYU/" + name.encode('utf-8') + "/" + filename.lstrip('/').encode('utf-8')
    ...
    def person_index_parse(self, response):
        ...
        if name == "":
            return
        filename = "../../../data/70-NYU/" + name.encode('utf-8') + "/index.html"
        print response.url,
        if os.path.isfile(filename):
            ...
            print " ..."
        system('mkdir "../../../data/70-NYU/' + name.encode('utf-8') + '"')
        ...
        for sel in response.xpath('//a/@href'):
            ...
            if url.find(":") == -1 and not (url[-4:] in [".pdf", ".avi", ".mp4", ".mov", ".jpg", ".png", ".ppt", ".zip", ".swf"])\
                    and not (url[-3:] in [".ps"]):
    ...
    def parse(self, response):
        for sel in response.xpath('//*[contains(concat(" ", normalize-space(@class), " "), " name ")]'):

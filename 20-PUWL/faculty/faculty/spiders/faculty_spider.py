...
class FacultySpider(scrapy.Spider):
    ...
    def person_info_parse(self, response):
        ...
        if filename == "":
            return
        target_filename = "../../../data/20-PUWL/" + name.encode('utf-8') + "/" + filename.lstrip('/').encode('utf-8')
    ...
    def person_index_parse(self, response):
        ...
        if name == "":
            return
        filename = "../../../data/20-PUWL/" + name.encode('utf-8') + "/index.html"
        print response.url,
        if os.path.isfile(filename):
            ...
            print " ..."
        system('mkdir "../../../data/20-PUWL/' + name.encode('utf-8') + '"')
    ...
    def parse(self, response):
        for sel in response.xpath('//*[@id="right-column"]/div/table[1]/tr[position()>1]'):
            name = sel.xpath("td[1]/text()").extract()[0]
            link = sel.xpath("td[6]//@href").extract()[0]
            if name.strip() == "":
                continue
            if link.find(":") == -1:

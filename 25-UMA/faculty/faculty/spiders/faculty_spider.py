...
class FacultySpider(scrapy.Spider):
    ...
    def person_info_parse(self, response):
        ...
        if filename == "":
            return
        target_filename = "../../../data/25-UMA/" + name.encode('utf-8') + "/" + filename.lstrip('/').encode('utf-8')
    ...
    def person_index_parse(self, response):
        ...
        if name == "":
            return
        filename = "../../../data/25-UMA/" + name.encode('utf-8') + "/index.html"
        print response.url,
        if os.path.isfile(filename):
            ...
            print " ..."
        system('mkdir "../../../data/25-UMA/' + name.encode('utf-8') + '"')
    ...
    def parse(self, response):
        for sel in response.xpath('//*[@id="block-system-main"]/div/div/div[2]/div/div'):
            name = sel.xpath('div[2]/div/div/text()').extract()[0]
            link = sel.xpath('div[1]/div/div/a/@href').extract()[0]
            if name.strip() == "":
                continue
            if link.find(":") == -1:
